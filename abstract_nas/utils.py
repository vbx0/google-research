...
def report_memory() -> None:
    ...
    logging.info("num_buffers: %d | total_elements: %.2e", len(live_buffers), total)
...
def canonicalize_tensor_name(name):
    ...
    For an op that produces only one output, we may refer to its output tensor
...
def write_to_store(obj, path):
    """Util for writing arbitrary objects to a store."""
    ...
    # temporary file and then move to the actual path name.
...
def read_from_store(path):
  """Utility function for reading arbitrary objects from store.
  ...
  def loads(bytes_object, pickle=builtin_pickle):
    """Returns an object unpickled from bytes.
    ...
    try:
      ...
      logging.exception(
          ...
          "This may be an old pickle written in Python 2, which is no longer "
          "supported. Please use the repickle script to update your pickles.")
      raise

  def load(file, pickle=builtin_pickle):
    """Returns an unpickled object from a file.
